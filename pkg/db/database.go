...
var conStr func(s helpers.IOWithAuth) string = func(s helpers.IOWithAuth) string {
	auth := s.Auth()
	connectData := struct {
		Username string
		Password string
...
func NewDatabase(c helpers.Config, name string) (*Database, error) {
	var src helpers.IOWithAuth
	var db *Database
